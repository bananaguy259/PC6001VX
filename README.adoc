...
=== コマンドラインオプション
PC6001VXはコマンドラインオプションをサポートしています。 +
以下のオプションが使用可能です。 +
[options="header"]
|================
|オプション|引数|説明
|-s +
--safemode |(なし)| セーフモード(ハードウェアアクセラレーションを無効)で起動します。
|-l +
--loadstate |(どこでもLOADファイル名)| 起動時に任意のどこでもLOADファイルを読み込んで起動します。
|-v +
...
. シェルにて `apt install build-essential libx11-dev mesa-common-dev libopengl-dev libsdl2-dev qtcreator qt6-base-dev qt6-multimedia-dev libavformat-dev libavcodec-dev libswscale-dev` を実行。 +
...
PAUSE/CTRL+F12キーを使うか、キーパネルを表示させてかなの切り替えを行ってください。
* Android
** スナップショット、テープのエクスポート機能が正常に機能しません。
** メニューのテキストの一部が中略されて表示されています。
** 最近のQtのバグ?により、タッチキーボードのキーを離した判定がうまくできません(マルチタッチの場合に顕著です)。SIMPLEモードでのゲームプレイが実用的でなくなっています。

== 更新履歴
.git master
* (Android)サウンドが乱れるのを修正
* (Android)Fixed choppy sound.
